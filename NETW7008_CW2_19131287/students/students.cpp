...
/* Main Funcion
    the main function
    It receives the Cookies paramters and shows all student and allows to add or delete them

    @POSTparam student
    the id of the student that will be delted; or the  name of the student that will be added
    @POSTparam action
    ...
    if (SessionAlive(username, SessionID)) {
        try {
            ...
            if (!fi->isEmpty() && fi != (*cgi).end()) {
                ...
                if (!action.empty() && !student.empty())
                    if (action == "add") {
                        InsterQuery("INSERT INTO students(name,active) VALUES ('" + student + "','1')");
                    }
                    else if (action == "del") {
                        InsterQuery("UPDATE students SET active = '0' WHERE idstudents = '" + student + "'");
                    }
            }

            #pragma endregion

            sql::ResultSet* res = MakeQuery("SELECT idstudents,name FROM students WHERE active ='1'");
            ...
            while (res->next()) {
                ...
                cout << p(res->getString("name")+", id: "+ res->getString("idstudents")).set("class", "listlabel");
                cout << "</td>" << endl;
                cout << "<td>" << endl;
                cout << input().set("type", "hidden").set("name", "student").set("value", res->getString("idstudents")) << endl;
                cout << "</td>" << endl;
            ...
            cout << form().set("method", "post") << endl;
            cout << "<td class='listtable-left'>" << endl;
            cout << input().set("type", "text").set("name", "student").set("class", "listinput_text") << endl;
            cout << "</td>" << endl;
            cout << "<td class='listtabel-right'>" << endl;
